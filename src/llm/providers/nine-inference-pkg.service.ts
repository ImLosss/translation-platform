...
@Injectable()
export class NineInferencePkgService implements LlmProvider {
  private readonly logger = new Logger(NineInferencePkgService.name);
  ...
  async generateTranslation(chatHistory: any[], isJsonFormat: boolean, model?: string): Promise<LlmResponse> {
    ...
    const apiKey = this.configService.get<string>('NINE_INFERENCE_PKG_APIKEY');
    
    if (!apiKey) {
      this.logger.error('NINE_INFERENCE_PKG_APIKEY tidak ditemukan di environment variables!');
  ...
  async isAvailable(): Promise<boolean> {
    const apiKey = this.configService.get<string>('NINE_INFERENCE_PKG_APIKEY');
    if (!apiKey) {
      this.logger.warn('NINE_INFERENCE_PKG_APIKEY tidak ditemukan di environment variables!');
      return false;
    }
    return true; 
  }
